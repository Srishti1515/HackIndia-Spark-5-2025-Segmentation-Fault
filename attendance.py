...
window.configure(background="skyblue")  # Dark theme
...
titl = tk.Label(
    window, text="CLASS VISION", bg="#1c1c1c", fg="blue", font=("Verdana", 27, "bold"),
)
...
ri = Image.open("UI_Image/face1.png")
ri=ri.resize((220,220))
r = ImageTk.PhotoImage(ri)
label1 = Label(window, image=r)
label1.image = r
label1.place(x=100, y=270)

ai = Image.open("UI_Image/face3.png")
ai=ai.resize((220,220))
a = ImageTk.PhotoImage(ai)
label2 = Label(window, image=a)
label2.image = a
label2.place(x=980, y=270)

vi = Image.open("UI_Image/face2.png")
vi=vi.resize((220,220))
...
def TakeImageUI():
    ...
    # image and title
    titl = tk.Label(
        ImageUI, text="Register", bg="#1c1c1c", fg="green", font=("Verdana", 30, "bold"),
    )
    ...
    def take_image():
        ...
        # def capture_images():
        #     #IPFS CONNECT TO LOCAL IPFS NODE
        #     try:
        #         ipfs_client = ipfshttpclient.connect() # Connect to local IPFS node
        #     except Exception as e:
        #         message.config(text=f"IPFS Connection Error: {e}")
        #         return
        #     cam = cv2.VideoCapture(0)
        #     haarcasecade = cv2.CascadeClassifier(haarcasecade_path)
        #     sampleNum = 0
        #     image_cids = []
